...
public class PeruAuthenticationServiceTest {
    ...
    public void testInitialize() {
        peruAuthenticationService.initialize();
    }
    ...
    public void sendOtp_withValidDetails_thenPass() throws SendOtpException {
        ...
        expectedResult.setMaskedEmail("mockedEmail@example.com");
        expectedResult.setMaskedMobile("XXXXXXXXXX");
        when(helperService.sendOtpMock(
                eq("transactionId"),
                eq("individualId"),
                eq(Arrays.asList("email", "sms")),
                eq(relyingPartyId),
                eq(clientId)
        )).thenReturn(expectedResult);
